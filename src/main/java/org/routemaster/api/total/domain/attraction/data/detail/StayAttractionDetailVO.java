    ...
    public static final class StayAttractionDetailVOBuilder {
        public StayAttractionDetailVOBuilder builder(JsonNode jsonNode) {
            ...
            jsonNode.get("items").get("item").forEach(item -> {
                ...
                this.detail.barbecue = item.get("barbecue").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "바베큐");
                this.detail.beautyFacility = item.get("beauty").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "뷰티");
                this.detail.beverage = item.get("beverage").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "음료");
                this.detail.bicycleRent = item.get("bicycle").asText().equals("1");
                this.detail.campfire = item.get("campfire").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "캠프파이어");
                this.detail.fitness = item.get("fitness").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "피트니스") || includeInSubfacility(this.detail.subFacility, "헬스");
                this.detail.karaoke = item.get("karaoke").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "노래방");
                this.detail.publicBath = item.get("publicbath").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "공용샤워");
                this.detail.publicPC = item.get("publicpc").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "pc");
                this.detail.sauna = item.get("sauna").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "사우나");
                this.detail.seminar = item.get("seminar").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "세미나");
                this.detail.sports = item.get("sports").asText().equals("1") || includeInSubfacility(this.detail.subFacility, "스포츠");
                this.detail.refundPolicy = item.get("refundregulation").asText().isEmpty() ? null : item.get("refundregulation").asText();
            });
            return this;
        }
        private boolean includeInSubfacility(String subFaility, String facility) {
            return subFaility != null && this.detail.subFacility.contains(facility);
        }
    }
